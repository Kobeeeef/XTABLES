...
class XTablesClient:
    ...
    def initialize_client(self, server_ip, server_port):
        ...
        self.close_socket()  # Ensure no dangling sockets
        ...
        with self.lock:  # Ensure thread-safety
            while not self.shutdown_event.is_set():
                try:
                    ...
                    # Initialize ZeroMQ sockets if enabled
                    if self.useZeroMQ and not (self.push_socket and self.sub_socket):
                    ...
                    # Restart the message listener if necessary
                    ...
                    self.logger.error(f"Connection error: {e}. Retrying in 2 seconds...")
    ...
    def handle_reconnect(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                self.close_socket()  # Ensure the previous socket is fully closed
                self.logger.info(f"Reconnecting to server {self.server_ip}:{self.server_port}...")
                self.initialize_client(self.server_ip, self.server_port)  # Reinitialize the client
                if self.isConnected:
                    self.logger.info("Reconnection successful.")
                    self.resubscribe_all()
                    return
            except Exception as e:
                self.logger.error(f"Reconnection failed: {e}. Retrying in 2 seconds...")
                time.sleep(2)
        ...
        try:
            ...
            if method_type == "INFORMATION":
                if len(parts) < 1:
                    ...
                    return
                self.client.send_data("null:INFORMATION " + ClientStats.ClientStatistics(self.client.version).to_json())
                return
            if len(parts) < 2:
